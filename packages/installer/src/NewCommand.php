...
namespace Mom\Installer\Console;
...
use Mom\Installer\Console\Concerns\ConfiguresPrompts;
...
class NewCommand extends Command
{
    ...
    /**
     * Determine if the application is using Mom 1 or newer.
     */
    public function usingMomVersionOrNewer(int $usingVersion, string $directory): bool
    {
    ...
    protected function configure(): void
    {
        $this
            ->setName('new')
            ->setDescription('Create a new Mom application')
            ...
            ->addOption('breeze', null, InputOption::VALUE_NONE, 'Installs the Mom Breeze scaffolding')
            ->addOption('jet', null, InputOption::VALUE_NONE, 'Installs the Mom Jetstream scaffolding')
...
|__/     |__/ \______/ |__/     |__/                 
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $commands = [
            $composer . " create-project mom/mom \"{$directory}\" {$version} --remove-vcs --prefer-dist --no-scripts",
        ...
        if (($process = $this->runCommands($commands, $input, $output))->isSuccessful()) {
            ...
            $output->writeln('');
            $output->writeln('  New to Mom? Check out our <href=https://bootcamp.mom.com>bootcamp</> and <href=https://mom.com/docs/installation#next-steps>documentation</>. <options=bold>Build something amazing!</>');
            $output->writeln('');
        }
    ...
    protected function promptForDatabaseOptions(string $directory, InputInterface $input)
    {
        ...
        if ( ! $input->getOption('database') && $input->isInteractive()) {
            ...
            $migrate = confirm(
                label: 'sqlite' !== $input->getOption('database')
    ...
    protected function promptForJetstreamOptions(InputInterface $input): void
    {
        ...
        collect(multiselect(
            ...
            default: array_filter([
                ...
                'inertia' === $input->getOption('stack') && $input->getOption('ssr') ? 'ssr' : null,
            ]),
        ))->each(fn ($option) => $input->setOption($option, true));
    }
    ...
    protected function installPest(string $directory, InputInterface $input, OutputInterface $output): void
    {
        ...
        $commands = [
            ...
            $composerBinary . ' require pestphp/pest pestphp/pest-plugin-mom --no-update --dev',
    ...
    /**
     * Create a Git repository and commit the base Mom skeleton.
     */
    protected function createRepository(string $directory, InputInterface $input, OutputInterface $output): void
    {
        ...
        $commands = [
            'git init -q',
            'git add .',
            'git commit -q -m "Set up a fresh Mom app"',
            "git branch -M {$branch}",
        ];
    ...
    protected function isParked(string $directory)
    {
        ...
        return false !== $output ? in_array(dirname($directory), json_decode($output)) : false;
    }
